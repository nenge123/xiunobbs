...
/*function t($name = '') {
	header("Time $name:".substr(microtime(1) - $_SERVER['REQUEST_TIME_FLOAT'], 0, 7));
}*/
...
function xn_debug_info() {
	...
	if(DEBUG > 1) {
		...
		$s .= '<p>Processed Time:'.(microtime(1) - $_SERVER['REQUEST_TIME_FLOAT']).'</p>';
		...
		$s .= "<tbody></table></div>".PHP_EOL;
		$s .= '<h3>REQUEST</h3>';
		$s .= PHP_EOL.'<div class="table-responsive"><table class="table table-dark table-striped text-keepall text-start" style="white-space: nowrap;"><thead><tr><th>offset</th><th>值</th></tr></thead><tbody>'.PHP_EOL;
		foreach($_REQUEST as $k=>$v):
			$s .= sprintf('<tr><td>%s</td><td>%s</td></tr>'.PHP_EOL,$k,$v);
		endforeach;
		$s .= "<tbody></table></div>".PHP_EOL;
		if(!empty($_SESSION)) {
			$s .= '_SESSION:<br>';
		...
		$s .= '<details close><summary style="font-size: large;margin-bottom: .5em;">调用文件('.count($files).')</summary><ul>';
		foreach($files as $file):
			$s.='<li><code>'.MyApp::convert_safe_path($file).'</code></li>';
		endforeach;
		$s .= '</ul></details>';
		$s .= '</fieldset>';
	}
...
function _SERVER($k, $def = '') { return isset($_SERVER[$k]) ? $_SERVER[$k] : $def; }
