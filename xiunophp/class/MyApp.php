...
class MyApp implements \ArrayAccess
{
	...
	public function __construct($conf = array())
	{
		...
		$this->datas = array(
			...
			'isIndexPath' => str_ends_with($_SERVER['SCRIPT_NAME'], 'index.php'),
		);
		$this->datas['longip'] = intval(ip2long($this->datas['ip']));
		define('APP_SITE', $this->convert_site(APP_PATH));
		$this->setConf($conf);
		set_exception_handler(fn($m)=>new lib\throwException($m));
	...
	{
		$arr = explode('\\', $class);
		$path =__DIR__ . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $arr) . '.php';
		if(is_file($path)):
			if($arr[0]=='model'):
		...
		$path = plugin::path($class.'/class/'.$class.'.php');
		if(is_file($path)):
			return require($path);
		endif;
		return false;
	...
	public function set_cookies_safe(string $name, mixed $data, int $time = 0)
	{
		if (empty($data)&&!is_int($data)):
			...
			$this->datas['cookies'][$name] = $data;
			$data = '__'.self::encrypt(strval($data));
		endif;
		return setcookie(
			$this->datas['cookie_prefix'] . $name,
			$data,
	...
	public function get_url_href(string|array $router = 'index', string|array $param = array(), bool $width = false, $top = false): string
	{
		...
		$rewritestyle = $this->datas['rewrite_style'];
		$rewriteopen = $this->datas['rewrite_open'];
		if ($top):
			#还原默认设置
			$rewriteroot = APP_SITE;
			$rewriteopen = self::conf('url_rewrite_on',false);
			$rewritestyle = self::conf('url_rewrite_style',false);;
			if(!$rewriteopen&&$rewritestyle):
				$rewriteroot .='index.php';
			endif;
		endif;
		if (empty($router)):
			$router = '';
			if ($rewriteopen && empty($router) && $this->datas['extension'] != 'php'):
				#设置 如index.html默认地址
		...
		#index.php 为引导文件
		if ($this->datas['isIndexPath'] && $rewriteopen):
			#是否支持伪静态重写
			if (!empty($param)):
				$router .= '?' . $param;
			endif;
	...
	public static function cookies_encrypt($name, ?string $value = null, int $time = 0)
	{
		self::app()->set_cookies_safe($name, $value, $time);
	}
