...
class plugin
{
	 ...
	 * 返回解析后的插件模板地址
	 */
	public static function tpl_link(string $file, ?string $dir = null, ?string $tmpfile=null): string
	{
	 ...
	 * 加载插件中的esm
	 */
	public static function js_module(string $name, string $dir = 'null'): string
	{
		if (empty($dir)):
			if (defined('PLUGIN_DIR')):
				$dir = PLUGIN_DIR;
		...
		return ' onmethods="loadmodule" module-url="' . self::site($dir . '/view/js/module/' . $name . '.js') . '"';
	}
}
