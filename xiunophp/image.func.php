...
function image_safe_name($filename, $dir) {
	...
	if(is_file($dir."$s1.$s2")) {
		$newname = $s1.$_SERVER['REQUEST_TIME'].rand(1, 1000).'.'.$s2;
	} else {
		$newname = "$s1.$s2";
	}
	return $newname;
...
function image_rand_name($k) {
	return $_SERVER['REQUEST_TIME'].'_'.rand(1000000000, 9999999999).'_'.$k;
}
...
function image_safe_thumb($sourcefile, $id, $ext, $dir1, $forcedwidth, $forcedheight, $randomname = 0) {
	...
	$filename = $randomname ? md5(rand(0, 1000000000).$_SERVER['REQUEST_TIME'].$ip).$ext : $id.$ext;
