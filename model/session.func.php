...
function sess_new($sid) {
	global $longip, $conf, $g_session, $g_session_invalid;
	...
	if($cookie_test) {
		...
		setcookie('cookie_test', '', $_SERVER['REQUEST_TIME'] - 86400, '');
	} else {
		$cookie_test = xn_encrypt(md5($agent.$longip), $conf['auth_key']);
		setcookie('cookie_test', $cookie_test, $_SERVER['REQUEST_TIME'] + 86400, '');
...
function sess_write($sid, $data) {
	global $g_session, $longip, $g_session_invalid, $conf;
	...
	$session_delay_update_on = !empty($conf['session_delay_update']) && $_SERVER['REQUEST_TIME'] - $g_session['last_date'] < $conf['session_delay_update'];
