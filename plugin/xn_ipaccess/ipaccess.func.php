...
function ipaccess_inc($ip, $action, $n = 1) {
	...
	if ($gid === 1 || (function_exists("vip_isvip") && boolval(vip_isvip($uid)))) {
		return true; //不增加数量
	}
	...
	if (empty($arr)) {
		...
		$arr = array($action . '+' => $n, 'actions+' => 1, 'last_date' => $_SERVER['REQUEST_TIME']);
...
function ipaccess_check($ip, $action) {
	...
	if (function_exists("vip_isvip") && boolval(vip_isvip($uid))) return TRUE;
