...
elseif($action == 'notice') {
	...
	if($method == 'GET') {
		...
		$noticelist = huux_notice::notice_find_by_recvuid($uid, $page, $pagesize, $type);
		$type != 0 AND $notices = huux_notice::notice_count(array('recvuid'=>$uid, 'type'=>$type));
		...
		if($act == 'readall') {
			// 全部已读
		   	$r = huux_notice::notice_update_by_recvuid($uid, array('isread'=>1));
			...
			$r = huux_notice::notice_delete($nid);
			$r === FALSE AND message(-1, lang('notice_my_update_failed'));
			...
			$noticelist = huux_notice::notice_find_by_nids($nidarr);

			foreach($noticelist as &$notice) {
				$nid = $notice['nid'];
				$recvuid = $notice['recvuid'];
				$uid == $recvuid AND huux_notice::notice_delete($nid);
			}
			message(0, lang('notice_delete_notice_sucessfully'));
