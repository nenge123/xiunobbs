...
// 发送验证码
	...
	$r = MyApp::xn_send_mail($smtp, $conf['sitename'], $email, $subject, $message);
...
*/
	...
	$_SERVER['HTTP_USER_AGENT'] != $_useragent AND message(-1, lang('authorized_get_failed'));
...
function user_auth_check($token) {
	...
	$_user = user_read($_uid);
	empty($_user) AND message(-1, lang('user_not_exists'));
	$_SERVER['REQUEST_TIME'] - $_time > 3600 AND message(-1, lang('link_has_expired'));
