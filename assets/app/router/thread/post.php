...
if(empty($thread)||empty($access['allowpost'])):
    #无权回帖 非法引用
    $myapp->data['errortitle'] = $myapp->getLang('post_unlogin');
    $myapp->data['errormessage'] = $myapp->getLang('post_unlogin_msg');
    include $myapp->template('thread/modal-error');
    $myapp->exit();
endif;
if($myapp->data['method']=='POST'):
    ...
    if($myapp->data['time'] - $posttime < $mintime):
        #操作过快
        $myapp->data['errortitle'] = $myapp->getLang('post_title');
        $myapp->data['errormessage'] = $myapp->getLang('post_message_too_fast',array('num'=>$mintime + $posttime - $myapp->data['time']));
        include $myapp->template('thread/modal-error');
    endif;
    if($myapp->data['time'] - $posttime > $maxtime ):
        #太久不操作 更新
        $myapp->data['errortitle'] = $myapp->getLang('post_title');
        $myapp->data['errormessage'] = $myapp->getLang('post_message_time_long',array('num'=>$mintime));
        include $myapp->template('thread/modal-error');
    endif;
    if($myapp->get_time_decode($posthash,$posttime)!=$threadhashkey):
        #验证失败
        $myapp->data['errortitle'] = $myapp->getLang('post_title');
        $myapp->data['errormessage'] = $myapp->getLang('post_hash_not_vaild',array('num'=>$posttime - $myapp->data['time']));
        include $myapp->template('thread/modal-error');
    endif;
    $size = settings_value('post_message_size',15);
    if(empty($message)||strlen($message)<$size):
        #回帖内容 太短
        $myapp->data['errortitle'] = $myapp->getLang('post_title');
        $myapp->data['errormessage'] = $myapp->getLang('post_no_message',array('size'=>$size));
        include $myapp->template('thread/modal-error');
    endif;
    ...
    if($maxsize>0&&strlen($message)>$maxsize):
        #回帖内容 太长
        $myapp->data['errortitle'] = $myapp->getLang('post_title');
        $myapp->data['errormessage'] = $myapp->getLang('post_message_too_long',array('maxsize'=>$maxsize,'size'=>strlen($message)));
        include $myapp->template('thread/modal-error');
    endif;
    #insert2json
    ...
    $myapp->data['errortitle'] = $myapp->getLang('post_success');
    $myapp->data['errormessage'] = $myapp->getLang('post_success_msg',array('maxsize'=>$maxsize,'size'=>strlen($message)));
    include $myapp->template('thread/modal-post-success');
endif;
#非法请求 本页面只允许POST请求,GET请求错误信息
$myapp->data['errortitle'] = $myapp->getLang('post_unable_visit');
$myapp->data['errormessage'] = $myapp->getLang('post_unable_visit_msg');
include $myapp->template('thread/modal-error');
$myapp->exit();