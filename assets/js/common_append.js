...
(function () {
    ...
    Object.assign(
        T.action, {
            "TAG-APP-GFM":(elm,status)=>{
                if(I.Attr(elm,'disable')) return;
                if(status=='connect'){
                        let href = I.Attr(elm,'href');
                        if(!href){
                        T.docload(async e=>{
                            elm.innerHTML = `<div class="markdown-github">${await T.RF('GetGFM',[elm.textContent])}</div>`;
                            elm.hidden = false;
                        });
                        }else{
                            if(/\.md$/.test(href)){
                                ...
                                T.FetchItem({
                                    ...
                                    type:'text',
                                    async success(text){
                                        elm.innerHTML = `<div class="markdown-github">${await T.RF('GetGFM',[text])}</div>`;
                                    }
                                })
                            }
                        }
            ...
            openForumTheadsEdit(btn){
                ...
                if(editThread.ispost){
                    return editThread.nWindow.RF('show');
                }
                if(tids.length){
                    T.I.toArr(tids,elm=>{
            ...
            ajaxWindow(elm){
                let href = I.Attr(elm,'href');
                let action = I.Attr(elm,'action');
                let ajax = I.Attr(elm,'ajax');
            ...
            ajax_editThread(elm){
                let method = I.Attr(elm,'method').toUpperCase();
                let action = I.Attr(elm,'action');
            ...
            ajax_deleteThread(elm){
                if(!elm.nWindow){
                    ...
                    elm.nWindow.action['submit'] = function(e){
                        ...
                        elm.ispost = true;
                        post = new FormData();
                        post.append('action',I.Attr(elm,'data-post'));
                        post.append('hash',I.Attr(elm,'data-hash'));
                        T.ajax({
                            url:location.href,
                            post,
                            type:'json',
            ...
            ajax_getPostList(elm){
                let url = I.Attr(elm,'href'),id = url.match(/thread-(\d+)-(\d+)/),post = new FormData(),page = parseInt(id[2]?id[2]:1),listprev=[],listnext=[],reHref= (v,page)=>v.replace(/thread-\d+-\d+/,'thread-'+id[1]+'-'+page);
            ...
            async qickPostBtn(elm,e){
                T.stopEvent(e);
                let postQuick = T.$('.quick-post'),pid=I.Attr(elm,'xn-pid');
            ...
            BindEvent(){
                T.$$('[xn-action]').forEach(elm=>{
                    let act = I.Attr(elm,'xn-action');
                    if(elm.tagName=="INPUT") T.on(elm,'change',e=>{T.RF(act,[elm,e])});
                    else T.on(elm,T.I.mobile?'touchend':'click',e=>{T.RF(act,[elm,e])});
                    elm.removeAttribute('xn-action');
                });
                T.$$('[ajax]').forEach(elm=>T.RF('ajaxWindow',[elm]));
            },
    ...
    T.upload_btn = (cb, value, meta) => {
        ...
        input.addEventListener('change', async (e) => {
            let result = await  T.RF('imagesPost',[e.target.files]);
            console.log(result);
            cb("");
            return;
